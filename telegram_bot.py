...
def get_global_data():
    ...
    for name, ticker in GLOBAL_MARKETS.items():
        try:
            # Menggunakan 5 hari agar aman dari hari libur bursa
...
def get_market_news():
    # Tambahkan timestamp (&_t=...) untuk mematahkan cache Google News
    current_time = int(time.time())
    url = f"https://news.google.com/rss/search?q=saham+IHSG+ekonomi+indonesia&hl=id&gl=ID&ceid=ID:id&_t={current_time}"
    items = []
    try:
        headers = {
